...
import { Calendar, User, IndianRupee, Package, AlertCircle, Sparkles } from "lucide-react";
    ...
    <div className="min-h-screen bg-black text-white pt-20 pb-16 px-4 md:px-6">
      <div className="max-w-[1380px] mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold">Buy Product Enquiries</h1>
        </div>
              ...
              <div className="relative space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 truncate">{enq.productName}</p>
                  </div>
                  <span className={`px-3 py-1.5 rounded-full text-xs font-medium ${getStatusColor(enq.status)}`}>
                    {enq.status}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-bold text-white">₹{enq.finalTotal.toLocaleString()}</span>
                  </div>
                  {enq.selectedAddons.length > 0 && (
                    <span className="text-xs text-purple-400">+ {enq.selectedAddons.length} Addons</span>
                  )}
                </div>
                ...
                <button
                  onClick={() => router.push(`/admin/bookings/productsbuying-requests/${enq._id}`)}
