...
export default function ProductDetailPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <p className="text-xl font-semibold text-blue-400 animate-pulse">Loading Product...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center flex-col gap-4">
        ...
        <Link href="/softwares-readymade" className="px-6 py-2.5 bg-blue-600 rounded-lg hover:bg-blue-700 transition text-sm font-medium">
          Back to Apps
        </Link>
      </div>
    );
  ...
  const handleCopyQuote = () => {
    ...
    if (selectedAddons.length > 0) {
      ...
      quote += `\n\nAdd-ons Total: ₹${Math.round(addonsTotal).toLocaleString()}`;
    }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <Link href="/softwares-readymade" className="inline-flex items-center gap-2 text-slate-400 hover:text-white transition mb-6 text-sm">
        ...
        <div className="bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 shadow-xl">
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 p-6 border-b border-slate-800">
          ...
          <div className="grid lg:grid-cols-2 gap-8 p-6">
            ...
            <div className="space-y-4">
                  ...
                  {product.screenshots.slice(0, 4).map((img, i) => (
                    <button
                      ...
                      className={`relative w-full h-20 rounded-lg overflow-hidden border-2 transition ${
                        mainImage === i ? 'border-blue-500' : 'border-slate-700 hover:border-slate-600'
                      }`}
                    >
              ...
              <div className="flex flex-wrap gap-2">
                {product.platforms?.map(p => (
                  <span key={p} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800/80 rounded-lg border border-slate-700 text-xs font-medium">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-800/50 p-5 rounded-xl border border-slate-700">
                <h3 className="text-lg font-bold mb-4">Customize Your Package</h3>
                ...
                {selectedAddons.length > 0 && (
                  <div className="mb-4 p-3 bg-slate-900/50 rounded-lg border border-slate-700">
                    <p className="text-xs font-semibold text-slate-400 mb-2">Selected Add-ons:</p>
                    <div className="space-y-1.5">
                      {selectedAddons.map(addon => (
                        <div key={addon.key} className="flex justify-between text-xs">
                          <span className="text-slate-300">{addon.label}</span>
                          <span className="font-medium text-white">₹{addon.cost.toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex gap-3 mt-5">
                  ...
                  <button 
                    ...
                    className="flex-1 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 rounded-lg font-medium shadow-lg transition text-sm"
                  >
                    Send Request
                  </button>
                </div>
