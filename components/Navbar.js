...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`z-50 w-full border-b border-blue-300/70 bg-black transition-all duration-300 ${pathname.startsWith("/admin") ? "hidden" : "block"}`}
      >
          ...
          <div className="flex items-center justify-between h-22">
            ...
            <div className="hidden md:flex items-center gap-2">
              {navLinks.map(({ name, href, icon: Icon }) => (
                ...
                >
                  <span
                    className={`absolute inset-0 bg-linear-to-r from-blue-600/0 via-sky-600/0 to-blue-600/0 group-hover:from-blue-600/20 group-hover:via-sky-600/30 group-hover:to-blue-600/20 transition-all duration-500 ${pathname === href ? "from-blue-600/30 via-sky-600/40 to-blue-600/30" : ""}`}
                  />
            ...
            <div className="hidden md:flex items-center gap-4">
              {user ? (
                <div className="relative" ref={dropdownRef}>
                  ...
                  <div
                    className={`absolute right-0 mt-3 w-60 bg-[#0a0a0a] border border-blue-500/30 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] overflow-hidden transition-all duration-300 z-[60] ${dropdownOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}`}
                  >
                    ...
                    <div className="p-2">
                      {isAdmin ? (
                        <Link
                          ...
                          className="flex items-center gap-3 px-4 py-3 text-sm text-gray-300 hover:text-white hover:bg-blue-600/10 rounded-xl transition-all group"
                        >
                          <LayoutDashboard
                            size={18}
            ...
            <button
              ...
              className="md:hidden p-3 rounded-xl bg-white/5 border border-blue-500/30 text-white"
            >
              {mobileOpen ? (
                <X className="w-6 h-6" />
              ) : (
        ...
        <div
          className={`md:hidden fixed inset-0 top-20 bg-black z-[100] transition-all duration-500 ${mobileOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}
        >
            ...
            <div className="pt-4 border-t border-white/10 mt-2">
              {user ? (
                <div className="space-y-3">
                  ...
                  {isAdmin ? (
                    <Link
                      href="/admin/dashboard"
                      onClick={() => setMobileOpen(false)}
                      className="flex items-center gap-4 px-6 py-4 rounded-2xl bg-blue-600/20 text-blue-400 font-bold"
                    >
                    ...
                    <Link
                      href="/softwares-readymade"
                      onClick={() => setMobileOpen(false)}
                      className="flex items-center gap-4 px-6 py-4 rounded-2xl bg-teal-600/20 text-teal-400 font-bold"
                    >
                      <ShoppingCart size={20} /> Buy Softwares
                    </Link>
                  )}
                  <button
                    onClick={handleLogoutClick}
                    className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl bg-red-600/10 text-red-400 font-bold border border-red-500/20"
                  >
                ...
                <button
                  ...
                  className="w-full py-4 bg-linear-to-r from-blue-600 to-teal-400 rounded-2xl font-bold text-white flex items-center justify-center gap-3"
                >
